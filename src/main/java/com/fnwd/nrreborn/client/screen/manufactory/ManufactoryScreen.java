package com.fnwd.nrreborn.client.screen.manufactory;
...
public class ManufactoryScreen extends AbstractContainerScreen<ManufactoryMenu> {
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (mouseX >= leftPos + 12 && mouseX <= leftPos + 24 && mouseY >= topPos + 12 && mouseY <= topPos + 75) {
            ...
            energyBarTooltip.add(Component.translatable("tooltip.nrreborn.energy_stored").withColor(16733695).append(Component.literal(MathUtils.getGUIEnergyText(menu.getEnergy(), menu.getCapacity())).withColor(16777215)));
            energyBarTooltip.add(Component.translatable("tooltip.nrreborn.speed_multiplier").withColor(5636095).append(Component.literal((menu.blockEntity.inventory.getStackInSlot(0).getCount() + 1) + "x").withColor(16777215)));
            energyBarTooltip.add(Component.translatable("tooltip.nrreborn.power_multiplier").withColor(5636095).append(Component.literal(BigDecimal.valueOf((double) ((menu.blockEntity.inventory.getStackInSlot(0).getCount() + 1) * (menu.blockEntity.inventory.getStackInSlot(0).getCount() + 1)) / Math.min(menu.blockEntity.inventory.getStackInSlot(0).getCount() + 1, menu.blockEntity.inventory.getStackInSlot(1).getCount() + 1))
                    .setScale(2, RoundingMode.HALF_UP)
                    .stripTrailingZeros().toPlainString() + "x").withColor(16777215)));
